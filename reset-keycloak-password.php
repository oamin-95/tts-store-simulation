...
if (!$subscriptionId) {
    echo "Usage: php reset-keycloak-password.php <subscription_id>\n";
    exit(1);
}
...
if (!$subscription) {
    echo "❌ الاشتراك #$subscriptionId غير موجود\n\n";
    exit(1);
}

if (!$subscription->keycloak_realm_id) {
    echo "❌ الاشتراك لا يحتوي على keycloak_realm_id\n\n";
    exit(1);
}
...
try {
    ...
    $client->put("/admin/realms/$realmId/users/$userId/reset-password", [
        ...
        'json' => [
            'type' => 'password',
            'value' => $newPassword,
            'temporary' => false, // Make it permanent this time
        ],
    ]);
    ...
    if (isset($meta['keycloak'])) {
        ...
        $meta['keycloak']['password_reset_at'] = now()->toISOString();
    ...
    echo "   ✅ كلمة المرور نهائية (ليست مؤقتة)\n\n";
